...
def fill_holes_in_object(obj, smooth_factor=0.2):
    ...
    bm = bmesh.new()
    bm.from_mesh(me)
    # Fill every boundary hole (including scan outer rim openings)
    ...
    bm.free()
    obj.data.update()
    # Recalc normals outward and enable Auto Smooth for cleaner shading
    bpy.ops.object.select_all(action='DESELECT')
    obj.select_set(True)
        ...
        for case_dir in sorted([p for p in jaw_dir.iterdir() if p.is_dir()]):
            ...
            for obj_path in obj_files:
                ...
                try:
                    ...
                    # Auto-fill boundary holes before material/orientation
                    fill_holes_in_object(model, smooth_factor=0.2)
