...
OUTPUT_DIR = Path("/home/user/lzhou/week4/multi_views5")
...
camera_views = {
    ...
    "left": (-140, -80, 80),         # From deeper left-rear-top angle
    "right": (140, -80, 80),        # From deeper right-rear-top angle
...
def setup_render_settings():
    ...
    scene.world.node_tree.links.new(bg.outputs['Background'], scene.world.node_tree.nodes['World Output'].inputs['Surface'])
...
def create_tooth_material():
    ...
    # Principled BSDF with subtle gloss for surface detail
    bsdf = nodes.new(type='ShaderNodeBsdfPrincipled')
    bsdf.inputs['Base Color'].default_value = (0.95, 0.92, 0.87, 1.0)  # Natural enamel tone
    bsdf.inputs['Roughness'].default_value = 0.25  # Slight gloss
    bsdf.inputs['Specular'].default_value = 0.4
    bsdf.inputs['Sheen'].default_value = 0.1  # Extra soft reflection on edges
        ...
        for patient_folder in [folder / "0AAQ6BO3"]:  # For testing, use a specific patient folder
            ...
            try:
                ...
                for view_name, camera_location in camera_views.items():
                    # Add extra lights for bottom view to enhance illumination from multiple lower angles
                    if view_name == "bottom":
                        for i, pos in enumerate([
                            ...
                            extra_light.name = f"{view_name}_light_{i}"
                            extra_light.data.energy = 14
                            extra_light.data.angle = 0.7

                    subdir_name = f"{patient_folder.name}-{jaw}"
                    subdir_path = OUTPUT_DIR / subdir_name
                    subdir_path.mkdir(parents=True, exist_ok=True)
                    out_path = subdir_path / f"{patient_folder.name}_{jaw}_{view_name}_light{config_id}.png"
