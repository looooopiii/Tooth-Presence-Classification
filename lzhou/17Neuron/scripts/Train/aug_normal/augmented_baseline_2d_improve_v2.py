...
OUTPUT_DIR = Path("/home/user/lzhou/week12/output/Train2D/aug_normal_17out_v2_strong_aug")
PLOT_DIR = Path("/home/user/lzhou/week12/output/Train2D/aug_normal_17out_v2_strong_aug/plots")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
PLOT_DIR.mkdir(parents=True, exist_ok=True)

# --- BCE Loss ---
BEST_MODEL_FILENAME = "bce_loss_full_dataset_best_17out_v2.pth"
LAST_MODEL_FILENAME = "bce_loss_full_dataset_last_17out_v2.pth"
PLOT_FILENAME = "bce_loss_full_dataset_metrics_17out_v2.png"
METRICS_FILENAME = "bce_loss_full_dataset_detailed_metrics_17out_v2.json"
# -----------------------------------
...
available_gpus = []
device = None
MIN_FREE_GPU_MB = 2000  # skip GPUs that have less than this free to avoid OOM on init


def pick_free_gpus(min_free_mb: int = MIN_FREE_GPU_MB, max_gpus: int = 1):
    """Return a list of GPU ids sorted by free memory (visible devices only)."""
    ...
    for idx in range(torch.cuda.device_count()):
        try:
            ...
            candidates.append((idx, free_mb, total_bytes // (1024 * 1024)))
        except RuntimeError:
            continue
    candidates.sort(key=lambda t: t[1], reverse=True)
    usable = [idx for idx, free_mb, _ in candidates if free_mb >= min_free_mb]
    if max_gpus:
        usable = usable[:max_gpus]
    return usable


def describe_gpu_mem(gpu_ids):
    """Human-friendly GPU free memory string for logging."""
    descriptions = []
    for idx in gpu_ids:
        try:
            free_bytes, total_bytes = torch.cuda.mem_get_info(idx)
            ...
            descriptions.append(f"{idx}: {free_mb}MB free / {total_mb}MB total")
        except RuntimeError:
            descriptions.append(f"{idx}: mem info unavailable")
...
def build_image_transform():
    return transforms.Compose(
        [
            ...
            # stronger color/lighting transformations
            transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1),

            # added random Gaussian blur
...
class ToothClassificationModel(nn.Module):
    ...
    def __init__(self, num_outputs=17, backbone: str = "resnet18"):
        ...
        self.dropout1 = nn.Dropout(0.5) # added stronger dropout (originally 0.3)
        self.fc2 = nn.Linear(512, 256)
        self.bn2 = nn.BatchNorm1d(256)
        self.dropout2 = nn.Dropout(0.5) # added stronger dropout (originally 0.3)
...
# train_epoch function modified to accept two loss functions
...
# train_epoch function modified to accept two loss functions
def validate_epoch(model, dataloader, criterion_teeth, criterion_jaw, device):
    model.eval()
    total_loss = 0.0
    all_preds_teeth, all_labels_teeth = [], []
    all_preds_jaw, all_labels_jaw = [], []
...
def main():
    ...
    if torch.cuda.is_available():
        available_gpus = pick_free_gpus()
        if len(available_gpus) == 0:
            print("CUDA is available but no GPU passed the free-memory check. Falling back to CPU.")
            device = torch.device("cpu")
        else:
            device = torch.device(f"cuda:{available_gpus[0]}")
            torch.cuda.set_device(available_gpus[0])
            print(
                f"CUDA is available. Visible devices: {torch.cuda.device_count()}. "
                f"Selected GPU(s): {describe_gpu_mem(available_gpus)}"
            )
    else:
        device = torch.device("cpu")
        available_gpus = []
    ...
    # use separate loss functions for teeth and jaw
    criterion_teeth = nn.BCEWithLogitsLoss().to(device)
    criterion_jaw = nn.BCEWithLogitsLoss().to(device)
    print(" Using separate BCEWithLogitsLoss for teeth (16) and jaw (1).")

    # added weight_decay (originally 0.01)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        val_metrics, val_preds_teeth, val_targets_teeth, val_preds_jaw, val_labels_jaw = validate_epoch(model, val_loader, criterion_teeth, criterion_jaw, device)
        scheduler.step()
