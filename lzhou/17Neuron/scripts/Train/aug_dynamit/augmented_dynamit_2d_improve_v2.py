...
OUTPUT_DIR = Path("/home/user/lzhou/week12/output/Train2D/aug_dynamit_17out_v2_strong_aug")
PLOT_DIR = Path("/home/user/lzhou/week12/output/Train2D/aug_dynamit_17out_v2_strong_aug/plots")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
PLOT_DIR.mkdir(parents=True, exist_ok=True)

BEST_MODEL_FILENAME = "dynamit_loss_full_dataset_best_17out_v2.pth"
LAST_MODEL_FILENAME = "dynamit_loss_full_dataset_last_17out_v2.pth"
PLOT_FILENAME = "dynamit_loss_full_dataset_metrics_17out_v2.png"
METRICS_FILENAME = "dynamit_loss_full_dataset_detailed_metrics_17out_v2.json"
...
class ToothClassificationModel(nn.Module):
    ...
    def __init__(self, num_outputs=17, backbone: str = "resnet18"):
        ...
        self.dropout1 = nn.Dropout(0.5) # added Dropout (originally 0.3)
        self.fc2 = nn.Linear(512, 256)
        self.bn2 = nn.BatchNorm1d(256)
        self.dropout2 = nn.Dropout(0.5) # added Dropout (originally 0.3)
        # --- output 17 vector ---
...
class CSVToothDataset(Dataset):
    def __init__(self, csv_file, root_dir, image_size=IMG_SIZE):
        ...
        for _, row in df.iterrows():
            ...
            if jaw is None: # if jaw is not found
                continue
    ...
    def _convert_32_to_17(self, labels_32: np.ndarray, jaw: str) -> np.ndarray:
        ...
        # fill in 16-dim tooth labels
        for idx_32, label_val in enumerate(labels_32):
            fdi = INDEX_TO_FDI_32[idx_32]

            # Check if FDI belongs to the current jaw
            if fdi in mapping:
                idx_16 = mapping[fdi]
                output_vector[idx_16] = label_val # fill in 16-dim tooth labels

        output_vector[NUM_TEETH_PER_JAW] = jaw_label # fill in 17th jaw label
...
class OriginalToothDataset(Dataset):
    ...
    def _load_labels_from_json(self, json_path, jaw):
        ...
        # build 32-dim missing teeth labels
...
def calculate_classification_metrics(predictions, targets):
    # only for teeth (first 16 outputs)
...
def train_epoch(model, dataloader, criterion_teeth, criterion_jaw, optimizer, device):
    ...
    metrics = {**metrics_teeth, **metrics_jaw} # merge dictionaries
...
def plot_training_curves(history, save_dir):
    ...
    fig.suptitle("2D Training Metrics (Full Dataset, 17 Out)", fontsize=16, fontweight="bold")
...
def main():
    ...
    # load datasets from CSVs and original renders
    datasets = []
    ...
    # setup loss functions, optimizer, scheduler
    criterion_teeth = Dynamit_Loss(device).to(device)
    criterion_jaw = nn.BCEWithLogitsLoss().to(device)
    print(" Using Dynamit_Loss (for teeth) + BCEWithLogitsLoss (for jaw).")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # add jaw_accuracy (use .get avoid KeyError)
