...
MODEL_PATH = "/home/user/lzhou/week12/output/Train2D/normal/resnet18_bce_best_2d.pth"
OUTPUT_DIR = "/home/user/lzhou/week12/output/Test2D/normal_17out"
...
def load_test_labels(csv_path):
    ...
    for _, row in df.iterrows():
        ...
        for tooth_fdi in mapping.keys(): # Only check FDIs for this jaw
            key = str(tooth_fdi)
            if key in df.columns and pd.notna(row[key]) and int(row[key]) == 1: # CSV 1 = missing
                idx16 = mapping[tooth_fdi]
                missing_teeth_vec[idx16] = 1.0 # 直接设置为 missing
...
def calculate_per_tooth_metrics(all_preds_17, all_targets_17):
    ...
    for fdi in all_fdi_labels:
        ...
        # Only care about the metrics for the "missing=1" class
        if len(f1) == 2: # ensure both classes are present
            f1_missing = f1[1]
            precision_missing = precision[1]
            recall_missing = recall[1]
            support_missing = int(support_array[1])
        else:
             # deal with edge case where only one class is present
            f1_missing = 0.0
...
def main():
    ...
    # save per-tooth metrics into the main metrics dict
    ...
    with open(Path(OUTPUT_DIR) / 'test_metrics_17out.json', 'w') as f:
        ...
        # per_tooth_metrics is already serializable
