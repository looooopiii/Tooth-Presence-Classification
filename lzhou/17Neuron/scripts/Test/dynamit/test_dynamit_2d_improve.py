...
# Point to the new 17-output Dynamit model
MODEL_PATH = "/home/user/lzhou/week12/output/Train2D/dynamit_17out/dynamit_loss_best_2d_17out.pth"

# Output directory for reports
OUTPUT_DIR = "/home/user/lzhou/week12/output/Test2D/dynamit_17out"
Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)

# Update to 17 outputs
# NUM_TEETH = 32
NUM_OUTPUTS = 17
...
class ResNetMultiLabel(nn.Module):
    # Change to num_outputs
    def __init__(self, backbone="resnet18", num_outputs=17, pretrained=False):
        super().__init__()
...
# This function is completely rewritten for 17 outputs
def load_test_labels(csv_path):
    ...
    CSV '1' means missing, '0' means present.
    """
    ...
    for _, row in df.iterrows():
        ...
        # Fill 16-teeth vector (1=missing)
        missing_teeth_vec = np.zeros(NUM_TEETH_PER_JAW, dtype=np.float32)
        for tooth_fdi in mapping.keys(): # Only check FDIs for this jaw
            ...
            if key in df.columns and pd.notna(row[key]) and int(row[key]) == 1:
                ...
                missing_teeth_vec[idx16] = 1.0
        
        # Assign to final 17-element vector
        label_vector[:NUM_TEETH_PER_JAW] = missing_teeth_vec
...
# Rewritten to calculate separate metrics
def calculate_metrics(preds, targets):
    ...
    # Calculate metrics
    teeth_metrics = calculate_classification_metrics(probs_teeth, targets_teeth)
...
def calculate_per_tooth_metrics(all_preds_17, all_targets_17):
    """
    Calculates per-tooth metrics (F1, Precision, Recall, Support) for each of the 32 FDI teeth.
    all_preds_17 and all_targets_17 shape: [N, 17]
    """
    
    # split predictions and targets
    preds_teeth_probs = all_preds_17[:, :NUM_TEETH_PER_JAW]
    targets_teeth = all_targets_17[:, :NUM_TEETH_PER_JAW]
    # use jaw label to determine upper/lower
    targets_jaw = all_targets_17[:, NUM_TEETH_PER_JAW].astype(int) # 0=Upper, 1=Lower

    # binarize predictions at 0.5 threshold
    preds_bin_teeth = (preds_teeth_probs > 0.5).astype(int)

    per_tooth_metrics = OrderedDict()
    all_fdi_labels = sorted(UPPER_FDI_TO_IDX16.keys()) + sorted(LOWER_FDI_TO_IDX16.keys())
    
    # iterate over all 32 FDI teeth
    for fdi in all_fdi_labels:
        mask = None
        idx16 = -1
        
        if fdi in UPPER_FDI_TO_IDX16:
            # select all upper jaw samples (targets_jaw == 0)
            mask = (targets_jaw == 0)
            idx16 = UPPER_FDI_TO_IDX16[fdi]
        elif fdi in LOWER_FDI_TO_IDX16:
            # select all lower jaw samples (targets_jaw == 1)
            mask = (targets_jaw == 1)
            idx16 = LOWER_FDI_TO_IDX16[fdi]
        
        if mask is None or np.sum(mask) == 0:
             # if no samples for this tooth, set metrics to zero
            per_tooth_metrics[fdi] = {
                'f1_missing': 0, 'precision_missing': 0, 'recall_missing': 0, 'support_missing': 0
            }
            continue

        # extract predictions and targets for this tooth
        tooth_preds = preds_bin_teeth[mask, idx16]
        tooth_targets = targets_teeth[mask, idx16]

        # calculate metrics for this tooth
        precision, recall, f1, support_array = precision_recall_fscore_support(
            tooth_targets, tooth_preds, average=None, labels=[0, 1], zero_division=0
        )
        
        if len(f1) == 2: # make sure class '1' exists
            f1_missing = f1[1]
            precision_missing = precision[1]
            recall_missing = recall[1]
            support_missing = int(support_array[1])
        else:
            f1_missing = 0.0
            precision_missing = 0.0
            recall_missing = 0.0
            support_missing = 0
...
# Rewritten for new metric structure
...
def print_sample_predictions(ids, preds, targets, num_samples):
    ...
    for i in sample_indices:
        ...
        # if jaw prediction is wrong, highlight
        if true_jaw_str != pred_jaw_str:
            print("   *** JAW MISMATCH: Tooth predictions are mapped to wrong jaw! ***")
        ...
        # only analyze tooth-level errors
        if true_jaw_str == pred_jaw_str:
...
def generate_test_plots(metrics, save_dir):
    ...
    plot_path = Path(save_dir) / "confusion_matrix_teeth.png"; plt.savefig(plot_path, dpi=300); plt.close()
    ...
    plt.ylabel('Ground Truth'); plt.xlabel('Prediction')
    plot_path = Path(save_dir) / "confusion_matrix_jaw.png"; plt.savefig(plot_path, dpi=300); plt.close()
    ...
    plt.title('Overall Precision-Recall Curve (Teeth Only)', fontsize=16, fontweight='bold'); plt.legend(loc="lower left"); plt.grid(True, alpha=0.5)
    plot_path = Path(save_dir) / "precision_recall_curve_teeth.png"; plt.savefig(plot_path, dpi=300); plt.close()
...
def main():
    ...
    def remap(sd):
        ...
        for k, v in sd.items():
            ...
            if nk.startswith('model.'):
                nk = nk[len('model.'):]
            if nk.startswith('backbone.'): # Training script used 'net', not 'backbone'
                nk = 'net.' + nk[len('backbone.'):]
            elif nk.startswith('resnet.'):
                nk = 'net.' + nk[len('resnet.'):]
            # This logic maps the training script's `self.net.fc` to this script's `self.net.fc`
            new[nk] = v
        return new
    ...
    # Inference (preds, targets are [N, 17])
    ...
    # Calculate per-tooth metrics
    print("\n" + "="*80 + "\n" + " "*28 + "PER-TOOTH METRICS (FDI)" + "\n" + "="*80)
    per_tooth_results = calculate_per_tooth_metrics(preds, targets)
    
    print(f"{'FDI Tooth':<10} | {'F1 (Missing)':<15} | {'Precision (Missing)':<20} | {'Recall (Missing)':<18} | {'Support (Missing)':<10}")
    print("-" * 80)
    # Sorted by FDI label
    for fdi_label in sorted(per_tooth_results.keys()):
        res = per_tooth_results[fdi_label]
        print(f"  {fdi_label:<8} | {res['f1_missing']:<15.4f} | {res['precision_missing']:<20.4f} | {res['recall_missing']:<18.4f} | {res['support_missing']:<10}")
    
    # add to metrics for saving
    metrics['per_tooth_metrics'] = {str(k): v for k, v in per_tooth_results.items()}


    # Plots
    print("\n" + "="*80 + "\n" + " "*28 + "GENERATING PLOTS" + "\n" + "="*80)
    # Pass only metrics dict
    generate_test_plots(metrics, OUTPUT_DIR)

    # Save 17-output results
    ...
    results_df.to_csv(Path(OUTPUT_DIR) / 'test_predictions_dynamit_17out.csv', index=False)

    # Save serializable metrics
    with open(Path(OUTPUT_DIR) / 'test_metrics_dynamit_17out.json', 'w') as f:
        ...
        # per_tooth_metrics already serializable
