...
MODEL_PATH = Path("/home/user/lzhou/week12/output/Train2D/aug_normal_17out_v2_strong_aug/bce_loss_full_dataset_best_17out_v2.pth")
OUTPUT_DIR = Path("/home/user/lzhou/week12/output/Test2D/aug_normal_17out_v2_strong_aug")
...
class ToothClassificationModel(nn.Module):
    ...
    # num_outputs: int = 17 for 16 teeth + 1 jaw
...
# Dataset for 2D tooth images and 17-dim labels
...
# load labels from CSV and convert to 17-dim vectors
def load_test_labels(csv_path: Path):
    ...
    for idx, row in df.iterrows():
        # make ID variants
        id_variants = set()
        primary_id_key = "" # need a key to infer the jaw
...
def calculate_per_tooth_metrics(all_preds_17, all_targets_17):
    ...
    # compute metrics for each tooth
    for fdi in all_fdi_labels:
        mask = None
        idx16 = -1
        
        if fdi in UPPER_FDI_TO_IDX16:
            mask = (targets_jaw == 0) # select upper jaw samples
            idx16 = UPPER_FDI_TO_IDX16[fdi]
        elif fdi in LOWER_FDI_TO_IDX16:
            mask = (targets_jaw == 1) # select lower jaw samples
        ...
        if len(f1) == 2:
            ...
            f1_missing, precision_missing, recall_missing, support_missing = 0.0, 0.0, 0.0, 0
...
# renewed metrics summary printout
...
# print sample predictions with jaw and teeth details
# enhanced to indicate jaw mismatches
def print_sample_predictions(ids, preds, targets, num_samples):
    ...
    for i in sample_indices:
        ...
        # Detailed analysis only if jaws match
        if true_jaw_str == pred_jaw_str:
...
def export_results(ids, preds, targets, save_dir: Path, metrics):
    ...
    csv_path = save_dir / "test_predictions_bce_17out.csv"
    results_df.to_csv(csv_path, index=False)

    # save serializable metrics
    json_path = save_dir / "test_metrics_bce_17out.json"
    ...
    # ensure per_tooth_metrics are also saved
    # (they were added to the metrics dict in the main function)
