...
def fill_holes_in_object(obj, smooth_factor=0.2):
    """Fill all boundary holes and lightly smooth the patched area (operator-free)."""
    me = obj.data
    bm = bmesh.new()
    bm.from_mesh(me)
    ...
    bm.to_mesh(me)
    bm.free()
    # Recalculate normals using data API (no mode switching)
    try:
        me.calc_normals()
    except Exception:
        pass
