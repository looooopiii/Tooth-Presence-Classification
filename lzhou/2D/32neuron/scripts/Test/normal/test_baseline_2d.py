...
TEST_LABELS_CSV = "/home/user/lzhou/week10/label_flipped.csv" # Ground truth labels for test set
MODEL_PATH      = "/home/user/lzhou/week15-32/output/Train2D/normal/resnet18_bce_best_2d.pth"
OUTPUT_DIR      = "/home/user/lzhou/week15-32/output/Test2D/normal"
...
# [STRATEGY: MAX]
# You selected "max_per_tooth".
# This is aggressive (High Recall), so we MUST find a high threshold to fix Precision.
...
# This default is just a placeholder now, as we will use Auto-Search.
...
def test_model_multiview(model, grouped_imgs, labels_dict, device, transform):
    ...
    for case_id in common_ids:
        ...
        # ---------------------------------------------------------------------
        if MULTIVIEW_STRATEGY == "max_per_tooth":
            # 1. Filter Top-K views (eliminate bad angles)
...
def main():
    ...
    # AUTO THRESHOLD
    # Use configurable metric and search range
    ...
    for fdi_label in VALID_FDI_LABELS:
        ...
        # Apply the BEST threshold to the CSV
        results[f'pred_{fdi_label}'] = (preds[:, idx] > best_threshold).astype(int)
        results[f'prob_{fdi_label}'] = preds[:, idx]
    
    # Filename includes the threshold
    results.to_csv(Path(OUTPUT_DIR) / f'test_predictions_multiview_{suffix}_th{best_threshold:.3f}.csv', index=False)

    with open(Path(OUTPUT_DIR) / f'test_metrics_multiview_{suffix}_th{best_threshold:.3f}.json', 'w') as f:
