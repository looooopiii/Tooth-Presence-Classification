...
# =================================================================================
TEST_IMG_DIR = Path("/home/user/lzhou/week15/render_output/test")
TEST_LABELS_CSV = Path("/home/user/lzhou/week10/label_flipped.csv")
MODEL_PATH = Path("/home/user/lzhou/week15-32/output/Train2D/aug_dynamit/dynamit_loss_full_dataset_best.pth")
OUTPUT_DIR = Path("/home/user/lzhou/week15-32/output/Test2D/aug_dynamit")
...
# Choose which metric to optimize: 'f1_micro' or 'f1_macro'
# For balanced per-tooth performance, use 'f1_macro'
OPTIMIZE_METRIC = 'f1_macro'

# Threshold search range - adjusted based on strategy
# For 'max_per_tooth': search high range [0.7, 0.99]
# For 'avg_all' or 'jaw_conf': search broader range [0.3, 0.8]
if MULTIVIEW_STRATEGY == "max_per_tooth":
    THRESHOLD_SEARCH_RANGE = (0.70, 0.995, 0.01)  # (start, end, step)
else:
    THRESHOLD_SEARCH_RANGE = (0.30, 0.85, 0.01)
...
def test_model_multiview(model, grouped_imgs, labels_dict, device, transform):
    ...
    print(f"✓ Intersection (test): {len(common_ids)} cases")
    if len(common_ids) == 0:
...
# =================================================================================
def find_optimal_threshold(preds, targets, metric='f1_micro', search_range=(0.30, 0.995, 0.01)):
    """
    Search for the threshold that maximizes the chosen metric.
    
    Args:
        metric: 'f1_micro' (default) or 'f1_macro'
        search_range: tuple of (start, end, step) for threshold search
    """
    ...
    print("\n" + "="*70)
    print(f" AUTO-THRESHOLD SEARCH (Maximizing {metric.upper()})")
    print(f" Search Range: [{start:.2f}, {end:.2f}], Step: {step:.3f}")
    print("="*70)
    print(f"{'Threshold':<10} {'Micro F1':<12} {'Macro F1':<12} {'Precision':<12} {'Recall':<10}")
    print("-" * 70)
    ...
    for th in thresholds:
        ...
        # Only print every 5th result to avoid clutter
        if abs(th - start) < step or abs(th - end) < step or int((th - start) / step) % 5 == 0:
            print(f"{th:<10.2f} {f1_micro:<12.4f} {f1_macro:<12.4f} {p_micro:<12.4f} {r_micro:<10.4f}")
        
        # Choose which metric to optimize
        current_score = f1_macro if metric == 'f1_macro' else f1_micro
    ...
    print("-" * 70)
    print(f" Best Threshold Found: {best_th:.3f} ({metric.upper()}: {best_score:.4f})")
    print("="*70 + "\n")
    return best_th
...
def calculate_metrics(preds, targets, threshold):
    """
    Calculate both micro and macro metrics - matches Aug Baseline.
    """
    if len(preds) == 0:
        return {}
    ...
    # Micro-averaged metrics (flatten everything)
    flat_preds = preds_bin.flatten()
    flat_targets = targets_bin.flatten()
    p_micro, r_micro, f1_micro, _ = precision_recall_fscore_support(
        flat_targets, flat_preds, average='binary', zero_division=0
    )
    acc_micro = accuracy_score(flat_targets, flat_preds)
...
def print_metrics_summary(metrics):
    ...
    print("  (Treating all predictions across all samples as one big binary classification)")
    ...
    print("  (Calculate per-tooth metrics first, then average across all 32 teeth)")
...
def print_sample_predictions(ids, preds, targets, num_samples, threshold):
    print(f"\n" + "="*80)
    print(f"SAMPLE PREDICTIONS (Threshold={threshold:.3f})")
    print("="*80)
    if len(ids) == 0:
        return
    ...
    for i in indices:
        ...
        print(f"\nCase: {case_id}")
        print(f"  Truth:  {sorted(list(truth)) if truth else '[]'}")
        print(f"  Pred:   {sorted(list(pred)) if pred else '[]'}")
...
def generate_test_plots(metrics, preds, targets, save_dir: Path, threshold):
    ...
    ax.set_title(f"Per-Tooth Performance (Aug Dynamit, Thresh={threshold:.3f})", fontsize=18, fontweight="bold")
    ax.set_xticks(x)
    ...
    plt.title(f"Confusion Matrix (Aug Dynamit, Thresh={threshold:.3f})", fontsize=16, fontweight="bold")
    ...
    plt.title("Overall Precision-Recall Curve (Aug Dynamit)", fontsize=16, fontweight="bold")
...
def export_results(ids, preds, targets, best_angles, jaw_confs, save_dir: Path, metrics, threshold):
    ...
    for fdi_label in VALID_FDI_LABELS:
        ...
        results_df[f"true_{fdi_label}"] = targets[:, idx].astype(int)
        results_df[f"pred_{fdi_label}"] = (preds[:, idx] > threshold).astype(int)
        results_df[f"prob_{fdi_label}"] = preds[:, idx]

    csv_path = save_dir / f"test_predictions_multiview_aug_dynamit_{suffix}_th{threshold:.3f}.csv"
    results_df.to_csv(csv_path, index=False)
    json_path = save_dir / f"test_metrics_multiview_aug_dynamit_{suffix}_th{threshold:.3f}.json"
...
def main():
    print("\n" + "="*80)
    print("AUG DYNAMIT TESTING - FIXED VERSION")
    print("="*80)
    print(f"Device: {DEVICE}")
    print(f"Strategy: {MULTIVIEW_STRATEGY}")
    ...
    print(f"✓ Labels: {len(labels_dict)} cases")
    print(f"✓ Images: {len(grouped_imgs)} case groups")

    transform = build_test_transform()

    # Inference
    print("\n[3/5] Running multi-view inference...")
    ...
    # Threshold Search
    print(f"\n[4/5] Searching optimal threshold...")
    ...
    print(f"\n[5/5] Calculating final metrics with threshold={best_threshold:.3f}...")
