...
OUTPUT_DIR = "/home/user/lzhou/week15-32/output/Test2D/32teeth_dynamit"
...
def normalize_png_stem_to_newid(stem: str) -> str:
    ...
    # Handle Jaw Type extraction - upper/lower
    jaw_key = ''
    lower_s = s.lower()
    
    if 'upperjawscan' in lower_s:
        # Extract numeric suffix (e.g., 001)
        match = re.search(r'upperjawscan(\d*)', lower_s, flags=re.IGNORECASE)
        suffix = match.group(1) if match else ''
        jaw_key = 'upper' + suffix  # Becomes "upper" or "upper001"
        ...
        suffix = match.group(1) if match else ''
        jaw_key = 'lower' + suffix  # Becomes "lower" or "lower001"
...
def print_metrics_summary(metrics):
    ...
    print("\n" + "="*80)
    print(" "*20 + "TESTING METRICS SUMMARY (Dynamic Loss)")
    print("="*80)
...
def generate_plots(metrics, preds, targets, save_dir):
    ...
    plt.title('Overall Confusion Matrix - Dynamic Loss')
    plt.xlabel('Predicted')
    plt.ylabel('Ground Truth')
    plt.savefig(Path(save_dir) / "confusion_matrix_dynamit.png", dpi=150)
    plt.close()
...
def main():
    print("\n" + "="*80)
    print(" "*10 + "2D MODEL TESTING (32 Teeth - Dynamic Loss)")
    print("="*80)
    ...
    metrics_file = Path(OUTPUT_DIR) / "test_metrics_dynamit.json"
