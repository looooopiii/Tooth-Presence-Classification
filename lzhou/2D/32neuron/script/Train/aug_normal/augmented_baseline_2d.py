...
OUTPUT_DIR = Path("/home/user/lzhou/week10/output/Train2D/aug_normal")
PLOT_DIR = Path("/home/user/lzhou/week10/output/Train2D/aug_normal/plots")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
PLOT_DIR.mkdir(parents=True, exist_ok=True)

# --- BCE Loss ---
BEST_MODEL_FILENAME = "bce_loss_full_dataset_best.pth"
LAST_MODEL_FILENAME = "bce_loss_full_dataset_last.pth"
PLOT_FILENAME = "bce_loss_full_dataset_metrics.png"
METRICS_FILENAME = "bce_loss_full_dataset_detailed_metrics.json"
# -----------------------------------
...
def calculate_classification_metrics(predictions, targets):
    ...
    # but train and validate loops have already applied sigmoid to logits
    # so the input predictions are already probabilities between 0-1
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for images, labels in tqdm(dataloader, desc="Training", leave=False):
        ...
        optimizer.zero_grad()
        logits = model(images)
        loss = criterion(logits, labels) # criterion now is BCEWithLogitsLoss
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
        all_preds.append(torch.sigmoid(logits.detach())) # keep sigmoid for metric calculation
...
def validate_epoch(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc="Validating", leave=False):
            ...
            loss = criterion(logits, labels) # criterion now is BCEWithLogitsLoss
            total_loss += loss.item()
            all_preds.append(torch.sigmoid(logits)) # keep sigmoid for metric calculation
...
def plot_training_curves(history, save_dir):
    ...
    fig.suptitle("2D Training Metrics (Baseline BCE Loss)", fontsize=16, fontweight="bold")
