...
MODEL_PATH = "/home/user/lzhou/week15-17/output/Train2D/16plus1teeth/baseline_bce_best_2d_16plus1teeth.pth"
OUTPUT_DIR = "/home/user/lzhou/week15-17/output/Test2D/16plus1teeth_auto_best"
...
# ========== SELECTION METRIC ==========
SELECTION_METRIC = 'macro_f1' # 'macro_f1', 'balanced_accuracy', 'macro_recall', 'macro_precision', 'accuracy'
...
# All valid FDI labels (for CSV reading)
...
def convert_csv_labels_to_16plus1(csv_row, jaw_type):
    ...
    Returns:
        17-dim vector: [16 tooth positions] + [jaw type]
    """
    ...
    if jaw_type == 'upper':
        for pos, fdi in enumerate(POSITION_TO_FDI_UPPER):
            ...
            if col_name in csv_row.index and pd.notna(csv_row[col_name]):
                ...
                tooth_missing[pos] = 0.0  # Default: present
...
def fuse_predictions_16plus1(probs_list, strategy=FusionStrategy.AVERAGE, n_best=2):
    ...
    if strategy == FusionStrategy.AVERAGE:
        ...
        # Use teeth confidence to select best angle
        confidences = [calculate_confidence_score_16plus1(p) for p in probs_list]
        best_idx = np.argmax(confidences)
        fused_teeth = teeth_probs[best_idx]
...
def load_test_labels(csv_path):
    ...
    for _, row in df.iterrows():
        ...
        if is_upper:
            ...
            jaw_type = 'unknown'
            continue  # Skip unknown jaw types
        
        # Convert to 16+1 format
...
def calculate_metrics_16plus1(preds, targets, jaw_type_dict, all_ids, preds_bin=None):
    ...
    # Per-position metrics (16 positions × 2 jaws = 32 entries)
    ...
    # Per-jaw classification accuracy (upper vs lower).
