...
LAST_MODEL_FILENAME = "dynamit_last_2d_16plus1teeth.pth"
...
# ==================== DYNAMIC LOSS (Modified for 16+1) ====================
class DynamitLoss16Plus1(nn.Module):
    """
    Combined loss for 16+1 architecture:
    - Dynamic Loss for teeth positions (0-15): batch-adaptive class balancing
    - Standard BCE for jaw classifier (16): balanced binary classification
    
    Rationale from meetings:
    - Teeth are imbalanced → use dynamic weighting
    - Jaw classes are balanced → use standard BCE
    """
    ...
    def forward(self, logits, targets):
        ...
        Args:
            ...
            targets: (B, 17) - ground truth labels
        
        Returns:
            Combined loss from teeth and jaw
        """
        ...
        # ===== Dynamic Loss for Teeth (positions 0-15) =====
        ...
        # ===== Standard BCE for Jaw (position 16) =====
        # Jaw classes are balanced (50-50 upper/lower in typical datasets)
...
class Tooth2DDataset(Dataset):
    ...
    def create_tooth_missing_vector_16plus1(self, vertex_labels_set, jaw_type):
        ...
        Logic consistent with 32-neuron version and 3D PointNet
        """
...
def plot_training_curves(history, save_dir, filename):
    ...
    fig.suptitle('ResNet18 2D Training - 16+1 Neurons with Dynamic Loss', fontsize=16, fontweight='bold')
...
def main():
    ...
    with open(metrics_file, 'w') as f:
        json.dump({
            ...
            'training_config': {
                'loss_function': 'DynamitLoss16Plus1 (Dynamic for teeth + BCE for jaw)',
