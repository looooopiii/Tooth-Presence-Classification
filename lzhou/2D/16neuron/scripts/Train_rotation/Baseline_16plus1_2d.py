...
# Outputs
OUTPUT_DIR = "/home/user/lzhou/week16-17/output/Train_rotation/16plus1teeth"
PLOT_DIR = "/home/user/lzhou/week16-17/output/Train_rotation/16plus1teeth/plots"
...
def get_free_gpus(threshold_mb=1000, max_gpus=2, verbose=True):
...
class Tooth2DDataset(Dataset):
    def __init__(self, img_roots, json_roots, image_suffix="_top.png", transform=None, is_training=True, cache_images=False):
        ...
        for jaw in ["lower", "upper"]:
            ...
            for case_dir in tqdm(case_dirs, desc=f"Loading {jaw}"):
                ...
                img_path = img_root / f"{case_id}_{jaw}{self.image_suffix}"
                json_path = case_dir / f"{case_id}_{jaw}.json"
                if img_path.exists() and json_path.exists():
                    try:
                        vertex_labels_set = self.load_labels(str(json_path))
                        tooth_labels = self.create_tooth_missing_vector_16plus1(vertex_labels_set, jaw)
                        img_data = None
                        if self.cache_images:
                            with Image.open(img_path) as img:
                                img_data = img.convert("RGB").copy()
                        self.data_cache.append({
                            "img": img_data if self.cache_images else str(img_path),
                            "targets": tooth_labels,
                            "jaw_type": jaw,
                            "case_id": case_id
                        })
                    except Exception as e:
                        print(f"Skipping {case_id} ({jaw}): {e}")
    ...
    def __getitem__(self, idx):
        ...
        jaw_type = sample["jaw_type"]
        tooth_labels = sample["targets"]
...
def main():
    ...
    class TransformedSubset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            img, tooth_labels, jaw_type = self.dataset[self.indices[idx]]
            
            if self.transform is not None:
                img = self.transform(img)
            
            return img, tooth_labels, jaw_type
