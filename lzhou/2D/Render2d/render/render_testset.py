...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Render top-view PNGs from PLY files.", add_help=False)
...
def main():
    ...
    if tmp_args.in_file:
        ...
        if render_all or (tmp_args.max is None):
            files = ply_files if render_all else ply_files[:1]
        else:
            files = ply_files[:max(1, tmp_args.max)]
    ...
    for ply_file in files:
        try:
            ...
            for angle_deg in ROTATION_ANGLES_DEG:
                ...
                out_name = f"{ply_file.stem}_top_rot{angle_deg}.png"
