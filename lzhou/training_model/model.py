...
# === CONFIGURATION ===
CSV_FILE = Path("/home/user/lzhou/week4/tooth_labels.csv")
...
def calculate_and_print_metrics(y_true, y_pred, tooth_labels):
    ...
    for i, tooth in enumerate(tooth_labels):
        ...
        balanced_accuracy = (sensitivity + specificity) / 2 if (sensitivity + specificity) > 0 else 0
    ...
    # Save per-tooth metrics to CSV
    metrics_df.to_csv(PLOTS_DIR / 'per_tooth_metrics.csv')
    print(f"Saved per-tooth metrics CSV to {PLOTS_DIR / 'per_tooth_metrics.csv'}")
    
    # Print overall metrics
    print("\n--- Overall Performance Metrics (Validation Set) ---")
    print(f"Macro F1-Score:    {f1_score(y_true, y_pred, average='macro', zero_division=0, pos_label=1):.4f}")
    print(f"Macro Precision:   {precision_score(y_true, y_pred, average='macro', zero_division=0, pos_label=1):.4f}")
    print(f"Macro Recall:      {recall_score(y_true, y_pred, average='macro', zero_division=0, pos_label=1):.4f}")
...
def plot_final_metrics(metrics_df):
    metrics_to_plot = metrics_df[['F1-Score', 'Sensitivity', 'Balanced Accuracy']]
    metrics_to_plot.plot(kind='bar', figsize=(20, 8), width=0.8)
    plt.title('Per-Tooth F1, Sensitivity, and Balanced Accuracy (Validation Set)')
    plt.ylabel('Score')
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'final_metrics_per_tooth_f1_sens_balacc.png')
    print(f"Saved final metrics plot to {PLOTS_DIR / 'final_metrics_per_tooth_f1_sens_balacc.png'}")
    plt.close()
