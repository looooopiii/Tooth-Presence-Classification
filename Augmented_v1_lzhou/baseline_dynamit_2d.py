...
# Outputs
OUTPUT_DIR = "/home/user/lzhou/week10/output/Train2D/dynamit"
PLOT_DIR = "/home/user/lzhou/week10/output/Train2D/dynamit/plots"
Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
Path(PLOT_DIR).mkdir(parents=True, exist_ok=True)

BEST_MODEL_FILENAME = "dynamit_loss_best_2d.pth"
LAST_MODEL_FILENAME = "dynamit_loss_last_2d.pth"
PLOT_FILENAME = "dynamit_loss_training_metrics_2d.png"
METRICS_FILENAME = "dynamit_loss_detailed_metrics_2d.json"
...
class Tooth2DDataset(Dataset):
    ...
    def _load_labels(self, json_path):
        with open(json_path, "r") as f:
            data = json.load(f)
        # 'labels' should be a list of present FDI teeth; convert to missing=1
        present = np.zeros(NUM_TEETH, dtype=np.float32)
        for fdi in data.get("labels", []):
    ...
    def __getitem__(self, idx):
        ...
        y = self._load_labels(sample["json"])
...
def plot_training_curves(history, save_dir, plot_filename):
    ...
    fig.suptitle('2D ResNet Training Metrics (Dynamit Loss)', fontsize=16, fontweight='bold')
    ...
    axes[1, 0].plot(epochs, history['train_acc'], 'b-', label='Train Acc')
    axes[1, 0].plot(epochs, history['val_acc'], 'r-', label='Val Acc')
