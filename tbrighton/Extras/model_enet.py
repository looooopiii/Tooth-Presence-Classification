...
#Unified Tooth Presence Classification Training Script (EfficientNet + Class Weights)
#This script trains a single EfficientNet-B0 model
...
PLOTS_DIR = Path("./plots_efficientnet")
...
# === 1. FOCAL LOSS IMPLEMENTATION (MODIFIED) ===
class FocalLoss(nn.Module):
    """
    Focal Loss now accepts `pos_weight` to handle class imbalance.
    `pos_weight` is a tensor of weights, one for each class.
    """
    ...
    def forward(self, inputs, targets):
        # Pass pos_weight to the underlying BCE loss
...
# === 2. CUSTOM PYTORCH DATASET (Unchanged) ===
...
def create_efficientnet_multilabel(num_classes):
    """
    Loads a pretrained EfficientNet-B0 and adapts it for multi-label classification.
    """
    # Using EfficientNet-B0. You can try other variants like b1, b2...
...
def calculate_class_weights(df, all_teeth): 
    """
    Calculates positive class weights to combat imbalance.
    rarer class (present wisdom tooth) -> higher weight.
    """
    tooth_cols = [f'tooth_{t}' for t in all_teeth]
    # Count how many times each tooth is PRESENT
    positive_counts = df[tooth_cols].sum()
    
    # Avoid division by zero for teeth that are never present in the training set
    positive_counts = positive_counts.replace(0, 1)

    # Calculate weight as the inverse of the frequency
    # This gives high weight to rare (low count) classes
    total_samples = len(df)
    pos_weights = total_samples / positive_counts
    
    # Normalize to keep weights from becoming too extreme
...
# (Metrics and plotting functions are unchanged from the previous script)
...
def plot_training_history(history):
    ...
    ax2.legend()
    ax2.grid(True)
    plt.savefig(PLOTS_DIR / 'training_history_enet.png')
    plt.close()
...
def plot_final_metrics(metrics_df):
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'final_metrics_per_tooth_enet.png')
    plt.close()

# === 5. TRAINING & VALIDATION LOOP (Unchanged) ===
def train_model(model, criterion, optimizer, dataloaders, num_epochs=25):
    # ... (same as before, including the .cpu() fix)
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), 'best_efficientnet_weighted_model.pth')
...
def main():
    print("Starting Tooth Classification: EfficientNet + Class Weights")
    ...
    # Calculate weights based on the TRAINING data only
    pos_weights = calculate_class_weights(train_df, ALL_TEETH).to(DEVICE)
    
    # Instantiate the weighted Focal Loss
    criterion = FocalLoss(
    ...
    model = create_efficientnet_multilabel(num_classes=NUM_CLASSES).to(DEVICE)
    ...
    print(f"\nTraining Finished! The best model saved as 'best_efficientnet_weighted_model.pth'")
