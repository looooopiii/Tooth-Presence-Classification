# baseline_model.py using 0 as present, 1 as missing for 32 permanent teeth in FDI notation
...
BEST_MODEL_FILENAME = "baseline_bce_best_1.pth"
LAST_MODEL_FILENAME = "baseline_bce_last_1.pth"
PLOT_FILENAME = "baseline_bce_training_metrics_1.png"
METRICS_FILENAME = "detailed_metrics_1.json"
...
# Early stopping parameters (Note: This is defined but not implemented in the provided training loop)
...
class ToothClassificationDataset(Dataset):
    ...
    def create_tooth_presence_labels(self, vertex_labels_set):
        """Create a binary label vector where 1=missing tooth, 0=present tooth."""
        # First, create a vector where 1 indicates a PRESENT tooth.
        ...
        # Invert the vector to meet the requirement: 1 = missing, 0 = present.
    ...
    def __getitem__(self, idx):
        ...
        # This now creates labels where 1=missing, 0=present
...
def calculate_per_tooth_metrics(pred, target, num_teeth=32):
    ...
    for tooth_idx in range(num_teeth):
        ...
        support = int(np.sum(tooth_target == 1)) # Support is count of positive class (missing teeth)
        per_tooth_metrics[fdi_label] = {'precision': precision, 'recall': recall, 'f1': f1, 'accuracy': accuracy, 'support': support}
    ...
    macro_metrics = {'macro_precision': macro_precision, 'macro_recall': macro_recall, 'macro_f1': macro_f1, 'macro_accuracy': macro_accuracy}
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # --- MODIFIED: Save the last model at the end of each epoch ---
        ...
        if val_metrics['f1'] > best_f1:
            ...
            # --- MODIFIED: Save the best model with the new filename ---
    ...
    # --- MODIFIED: Use dynamic filename for metrics json ---
    metrics_file = Path(OUTPUT_DIR) / METRICS_FILENAME
    with open(metrics_file, 'w') as f:
        # Convert OrderedDict to a regular dict for JSON serialization if needed, though json.dump handles it
    ...
    # --- MODIFIED: Pass dynamic filename to plot function ---
