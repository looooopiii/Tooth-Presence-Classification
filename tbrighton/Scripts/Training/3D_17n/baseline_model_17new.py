...
# DIRECTORIES
OUTPUT_DIR = "/home/user/tbrighton/Scripts/Training/3D_17n/trained_models_baseline"
PLOT_DIR = "/home/user/tbrighton/Scripts/Training/3D_17n/plots_baseline"
Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
Path(PLOT_DIR).mkdir(parents=True, exist_ok=True)

# FILENAMES
BEST_MODEL_FILENAME = "baseline_model_best.pth"
LAST_MODEL_FILENAME = "baseline_model_last.pth"
...
def plot_per_tooth_metrics(metrics_dict, save_path):
    ...
    # Color code: Blue for Upper (1x, 2x), Orange for Lower (3x, 4x)
    colors = ['#1f77b4' if int(x) < 30 else '#ff7f0e' for x in fdi_labels]
    
    bars = plt.bar(fdi_labels, f1_scores, color=colors, alpha=0.8)
    
    plt.axhline(y=np.mean([s for s in f1_scores if s > 0]), color='r', linestyle='--', alpha=0.5, label='Mean Valid F1')
    plt.title('Per-Tooth F1 Score (Detection Performance)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(save_path / "per_tooth_f1.png", dpi=300)
    plt.close()

def plot_confusion_matrices(y_true, y_pred, save_path):
    """Generates Normalized Confusion Matrix"""
    ...
    plt.ylabel('Actual')
    plt.tight_layout()
    plt.savefig(save_path / "confusion_matrix_teeth.png", dpi=300)
    plt.close()

# =========================================
#  UTILS & OPTIMIZED DATASET
# =========================================
...
def print_final_report(model, loader, device):
    ...
    # Lists to store metrics ONLY for valid (support > 0) teeth
    ...
    for section_name, fdi_list, jaw_label in [("UPPER", UPPER_FDI, 0.0), ("LOWER", LOWER_FDI, 1.0)]:
        ...
        for fdi in fdi_list:
            ...
            # Collect flat lists for Confusion Matrix (only valid jaw data)
            ...
            if support > 0:
                ...
                # Do NOT add to averages if support is 0
    ...
    with open(Path(OUTPUT_DIR) / METRICS_FILENAME, 'w') as f:
        json.dump({
            'overall_f1': m_f1, 'overall_accuracy': m_acc, 'jaw_accuracy': jaw_acc,
...
def main():
    ...
    print(f"\n[3/5] Training for {NUM_EPOCHS} epochs (BASELINE: Standard BCE Loss, 4096 pts)...")
    best_f1 = 0.0
