#baseline model pointnet trained with dynamit loss function
#labels flipped 0 for presence, 1 for absence
...
# --- Updated filenames for this run ---
BEST_MODEL_FILENAME = "dynamit_loss_best_1.pth"
LAST_MODEL_FILENAME = "dynamit_loss_last_1.pth"
PLOT_FILENAME = "dynamit_loss_training_metrics_1.png"
METRICS_FILENAME = "dynamit_loss_detailed_metrics_1.json"
...
class ToothClassificationDataset(Dataset):
    ...
    def normalize_point_cloud(self, points):
        ...
        if max_dist > 0: points_centered /= max_dist
    ...
    def create_tooth_presence_labels(self, vertex_labels_set):
        ...
        for fdi_label in vertex_labels_set:
            ...
            if index is not None: tooth_presence[index] = 1.0
        return 1.0 - tooth_presence

    def sample_points(self, points):
        num_vertices = len(points)
        if num_vertices == 0: return np.zeros((self.num_points, 3), dtype=np.float32)
...
def calculate_per_tooth_metrics(pred, target, num_teeth=32):
    ...
    for tooth_idx in range(num_teeth):
        ...
        tooth_pred = pred[:, tooth_idx]; tooth_target = target[:, tooth_idx]
...
def plot_training_curves(history, save_dir):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(15, 12)); fig.suptitle('PointNet Training Metrics (Dynamit Loss)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout(); save_path = Path(save_dir) / PLOT_FILENAME; plt.savefig(save_path, dpi=300); plt.close()
...
def main():
    ...
    criterion = Dynamit_Loss(device)
    print("✓ Using Dynamit_Loss for class imbalance.")
