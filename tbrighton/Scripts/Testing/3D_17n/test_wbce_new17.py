...
# Model Path (Weighted BCE)
MODEL_PATH = "/home/user/tbrighton/Scripts/Training/3D_17n/trained_models_weighted/weighted_model_last.pth"

# Output Directory
OUTPUT_DIR = "/home/user/tbrighton/Scripts/Testing/3D_17n/weighted_bce_results"
...
# =================================================================================
# MODEL ARCHITECTURE (Must match Weighted BCE Training)
# =================================================================================
...
def plot_per_tooth_metrics(metrics_dict, save_path):
    ...
    plt.title(f'Weighted BCE Test: Per-Tooth F1 Score', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(Path(save_path) / "weighted_per_tooth_f1.png", dpi=300)
    plt.close()
...
def plot_confusion_matrices(y_true, y_pred, save_path, title_suffix="Teeth"):
    ...
    plt.ylabel('Actual')
    plt.tight_layout()
    plt.savefig(Path(save_path) / f"weighted_confusion_matrix_{title_suffix.lower()}.png", dpi=300)
    plt.close()
...
def calculate_and_print_metrics(all_data):
    ...
    print("\n" + "="*90)
    print("📊 WEIGHTED BCE OVERALL SUMMARY")
    print("=" * 90)
    ...
    with open(Path(OUTPUT_DIR) / "weighted_metrics.json", 'w') as f:
        json.dump({
            'overall_f1': m_f1, 
