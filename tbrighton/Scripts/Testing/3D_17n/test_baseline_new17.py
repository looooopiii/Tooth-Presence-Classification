...
OUTPUT_DIR = "/home/user/tbrighton/Scripts/Testing/3D_17n/baseline_results"
...
def plot_per_tooth_metrics(metrics_dict, save_path):
    ...
    plt.title(f'Test Set: Per-Tooth F1 Score (Rotation: {BEST_ROT})', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(Path(save_path) / "test_per_tooth_f1.png", dpi=300)
    plt.close()
...
def plot_confusion_matrices(y_true, y_pred, save_path, title_suffix="Teeth"):
    ...
    plt.ylabel('Actual')
    plt.tight_layout()
    plt.savefig(Path(save_path) / f"test_confusion_matrix_{title_suffix.lower()}.png", dpi=300)
    plt.close()
...
def calculate_and_print_metrics(all_data):
    ...
    # --- TOOTH DETECTION METRICS (Including Balanced Accuracy) ---
