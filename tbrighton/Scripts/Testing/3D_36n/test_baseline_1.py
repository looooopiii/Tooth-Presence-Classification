# testing file for baseline_model_1.py
# labels are flipped: 1=missing tooth, 0=present tooth
...
MODEL_PATH = "/home/user/tbrighton/Scripts/Training/3D_17n/trained_models_baseline/baseline_model_last.pth"
# ---------------
...
def align_point_cloud_robust(points):
    ...
    return points_centered @ rotation_matrix.T

def load_ply_file(ply_path):
    """Safely loads vertices from a .ply file."""
    try:
...
def normalize_point_cloud(points):
    ...
    # This was already centered by align_point_cloud_robust
...
def load_test_labels(csv_path):
    """Loads ground truth labels from the CSV file into the correct vector format."""
    df = pd.read_csv(csv_path)
    df['new_id'] = df['new_id'].astype(str).str.strip()
    labels_dict = {}
    
    # Convert FDI column names from int to string for robust lookup
    ...
    for _, row in df.iterrows():
        ...
        for tooth_fdi in VALID_FDI_LABELS:
            ...
            # The CSV now has 1 for missing, so we read it directly
...
def test_model(model, test_data, device):
    """Runs inference on the prepared test data."""
    model.eval()
...
def print_sample_predictions(ids, preds, targets, num_samples):
    ...
    for i in sample_indices:
        case_id = ids[i]
        target_labels = targets[i]; pred_labels = (preds[i] > 0.5).astype(int)
        
        # Get sets of FDI labels for missing teeth
        ...
        if case_id in labels_dict:
            ...
            if len(points) > 0:
                ...
                if "lower" in case_id.lower():
                    aligned_points[:, 1] *= -1; aligned_points[:, 2] *= -1
