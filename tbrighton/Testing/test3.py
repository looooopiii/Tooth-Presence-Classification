...
def main():
    ...
    if missing_in_folder:
        print(f"Missing in folder ({len(missing_in_folder)}): {list(missing_in_folder)[:5]} ...")
    if missing_in_df:
        print(f"Extra in folder ({len(missing_in_df)}): {list(missing_in_df)[:5]} ...")
    
    # Keep only rows that have a matching image
    df = df[df["new_id"].isin(matches)].reset_index(drop=True)
    print(f"Proceeding with {len(df)} matched samples")
